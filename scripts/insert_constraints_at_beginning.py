...
def decode_input(ntm, decoder, source, length_factor=1.3):
    ...
    # just get the best translation without constraints
    source_, constraints_ = ntm.build_input_representations(source, [])
...
def insert_constraints(source_file, target_file, config_file, output_dir, constraints):
    ...
    for i, (source, constraint) in enumerate(zip(source_seqs, constraints)):
        trans, score = decode_input(ntm, decoder, source)
        ...
        # insert each constraint at the beginning of the translation
        for c_i in constraint:
            trimmed_trans = c_i + trimmed_trans
